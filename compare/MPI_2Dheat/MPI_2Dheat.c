...
#define TIME_STEPS 1 /* number of time steps */
...
nodes after TIME_STEPS */
  if (taskid == MASTER)
      ...
      for (i=1; i<=numworkers; i++)
    {
      ...
      stopclock = clock();
      double timeperstep =((double)(stopclock-startclock))/CLOCKS_PER_SEC;
      timeperstep = timeperstep / TIME_STEPS;
      timeperstep = timeperstep / (NXPROB*NYPROB);
